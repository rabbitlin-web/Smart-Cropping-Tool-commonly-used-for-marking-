...
**增加了批量处理功能,可以按ctrl批量选择图片(最好这些图片大小一致,要切割的行列等一致,比如你有banana生成的批量同尺寸分镜头图集),然后调节行列等参数,而后进行切割,切割后会将这些图片打包到一起(但是里面会将图片放到不同文件夹)**
**Added batch processing function, you can press ctrl to batch select images (preferably these images are the same size, the columns to be cut are the same, etc., for example, you have a batch of the same size storyboard generated by banana), and then adjust the parameters such as columns, and then cut, these pictures will be packaged together after cutting (but the images will be put into different folders inside)**
